...
[The SLSA Provenance Build Definition](https://slsa.dev/provenance/v1#buildDefinition)
...
For the time being, our solution provides a stop gap until GitHub offers a native solution as per the issue above, [feat: include workflow inputs in externalParameters](https://github.com/actions/attest-build-provenance/issues/55) — now tracked upstream in [actions/toolkit#2331](https://github.com/actions/toolkit/issues/2331) (the `attest-build-provenance/predicate` action that previously allowed patching this in has since been discontinued) — to record/attest to user workflow inputs.
